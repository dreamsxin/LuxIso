...
async function createScene(): Promise<Scene> {
  ...
  next.addObject(new Lantern({
    id: 'lantern-west', x: 4.75, y: 3.25, glowColor: '#ffd166', postColor: '#40504b', heightPx: 52,
  }));
  next.addObject(new Lantern({
    id: 'lantern-east', x: 7.5, y: 3.25, glowColor: '#ffcf73', postColor: '#40504b', heightPx: 52,
  }));
  next.addObject(new Cloud({
    id: 'cloud-a', x: 2.2, y: 6.7, altitude: 5.2, speed: 0.22, angle: -0.18, scale: 0.8, seed: 0.42,
  }));
  ...
  next.addObject(new FloatingText({
    id: 'garden-label', x: 6.15, y: 3.25, z: 74, text: 'LANTERN GARDEN', color: '#fff0b0',
    duration: 1_000_000, speed: 0, fontSize: 12,
  }));
  ...
  next.addLight(new OmniLight({
    id: 'lantern-west-light', x: 4.75, y: 3.25, z: 52, radius: 170, color: '#ffd166',
    intensity: 0.78, falloff: 'quadratic',
  }));
  next.addLight(new OmniLight({
    id: 'lantern-east-light', x: 7.5, y: 3.25, z: 52, radius: 170, color: '#ffcf73',
    intensity: 0.78, falloff: 'quadratic',
  }));
  next.addLight(new OmniLight({
...
function bindControls(): void {
  ...
  for (const button of document.querySelectorAll<HTMLButtonElement>('.mode')) {
    button.addEventListener('click', () => {
      ...
      viewport.dataset.mode = mode;
      for (const candidate of document.querySelectorAll('.mode')) {
