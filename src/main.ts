...
function spawnFx(x: number, y: number, preset: FxPreset, color?: string, _count?: number): void {
  ...
  switch (preset) {
    case 'crystal':
      ps.addEmitter({
        rate: 0, life: [0.4, 0.8] as [number, number], speed: [2, 5] as [number, number],
        size: [4, 8] as [number, number], color: color ? [color] : ['#00ffff', '#ffffff'], gravity: 10,
      });
      break;
    case 'dust':
      ps.addEmitter({
        rate: 0, life: [0.6, 1.0] as [number, number], speed: [0.5, 2] as [number, number],
        size: [8, 20] as [number, number], color: color ? [color] : ['#887766'], gravity: 0,
      });
      break;
    case 'coin':
      ps.addEmitter({
        rate: 0, life: [0.8, 1.5] as [number, number], speed: [1, 4] as [number, number],
        size: [5, 10] as [number, number], color: ['#ffff00', '#ffd700'], gravity: 12,
      });
      break;
    default:
      ps.addEmitter({
        rate: 0, life: [0.3, 0.6] as [number, number], speed: [4, 8] as [number, number],
        size: [2, 5] as [number, number], color: color ? [color] : ['#ffffff', '#ffffcc'], gravity: 5,
      });
      break;
  }
...
function maybeSpawnFootstep(dt: number): void {
  ...
  ps.addEmitter({
    rate: 0, life: [0.3, 0.6], speed: [0.3, 0.9], size: [2, 4],
    color: ['#c8b090', '#a09070'], gravity: 0,
  });
  ps.onExhausted = () => scene.removeById(id);
  ps.burst(4);
  scene.addObject(ps);
...
function resetScene(): void {
  ...
  liveProps.clear();
  propsAlive = 0;
  score = 0; combo = 0; comboTimer = 0; waveNumber = 1;
...
function handlePointerDown(cx: number, cy: number): void {
  ...
  const world = scene.camera.screenToWorld(
    cx, cy, canvasW, canvasH, TILE_W, TILE_H, engine.originX, engine.originY, scene.view
  );
  ...
  ts => {
    ...
    if (!(document.activeElement instanceof HTMLInputElement)) {
      ...
      if (input.isDown('ArrowUp')) {
        character.position.y = clamp(character.position.y - KEY_STEP * 0.1, 0.5, ROWS - 0.5);
      }
      if (input.isDown('ArrowDown')) {
        character.position.y = clamp(character.position.y + KEY_STEP * 0.1, 0.5, ROWS - 0.5);
      }
      if (input.isDown('ArrowLeft')) {
        character.position.x = clamp(character.position.x - KEY_STEP * 0.1, 0.5, COLS - 0.5);
      }
    ...
    if (rainbowMode) {
      ...
      lightColorPicker.value = `#${hex(r)}${hex(g)}${hex(b)}`;
    }
