...
export class Cloud extends IsoObject {
  ...
  /**
   * Frame clock for the drift integration.
   *
   * Its `null` baseline is the point: `Engine` starts its clock at 0, so a 0
   * sentinel stayed armed after the first frame and every frame afterwards was
   * treated as the first — the cloud never drifted in a scene that began at
   * timestamp 0.
   */
  ...
  update(ts?: number): void {
    ...
    // Clamped to [0, 100 ms] by the shared clock: a hidden tab must not teleport
    // the cloud across the scene on the frame it comes back, and a rewound clock
