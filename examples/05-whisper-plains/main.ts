...
import {
  buildDeepSeaScene,
  DEEP_COLS, DEEP_ROWS,
  DEEP_PORTAL_X, DEEP_PORTAL_Y,
  DEEP_SPAWN_X, DEEP_SPAWN_Y,
} from './scenes/DeepSeaScene';
...
const sceneLabel = hud.addLabel({
  id: 'scene-name', x: 22, y: 33, text: '低语草原',
  color: '#e8f4e8', fontSize: 15, shadow: true,
});
const hintLabel = hud.addLabel({
  id: 'hint', x: 22, y: 54, text: 'WASD / 点击 移动',
  color: 'rgba(200,230,200,0.6)', fontSize: 10, shadow: true,
});
const portalHint = hud.addLabel({
  id: 'portal-hint', x: canvas.width - 200, y: canvas.height - 20,
  text: '✦ 传送阵感应到你的存在…',
  color: 'rgba(180,140,255,0)', fontSize: 11, shadow: true, visible: false,
});
const timeLabel = hud.addLabel({
  id: 'time', x: canvas.width - 80, y: 30, text: '☀ 白天',
  color: 'rgba(255,240,180,0.85)', fontSize: 11, shadow: true,
});
...
const {
  scene: lakeScene, lake: waveLake, portal: lakePortal, collider: lakeCollider,
} = buildLakeScene(LAKE_COLS, LAKE_ROWS);
...
function spawnBurst(scene: Scene, x: number, y: number): void {
  ...
  ps.addEmitter({
    maxParticles: 70, rate: 0, shape: 'ring',
  ...
  ps.addEmitter({
    maxParticles: 35, rate: 0, shape: 'point',
...
mgr.register('plains', () => ({
  ...
  onUpdate(dt, inp) {
    ...
    // 移动
    plainsMover.update(
      dt, inp, map, plainsScene.camera, plainsScene.tileW, plainsScene.tileH,
      engine.originX, engine.originY, canvas.width, canvas.height,
    ...
    if (n < 0.5) {
      _dreamTimer += dt;
      if (_dreamTimer > 0.5) { _dreamTimer = 0; _spawnDream(plainsScene, PLAINS_COLS, PLAINS_ROWS); }
    }
  },
  onDrawBackground: (ctx, w, h, ts) => drawPlainsSky(ctx, w, h, ts, dayNight),
  onDrawOverlay: (ctx, _w, _h, ts) => plainsMover.drawMarker(
    ctx, plainsScene.camera, plainsScene.tileW, plainsScene.tileH,
    engine.originX, engine.originY, ts
  ),
}));
...
mgr.register('lake', () => ({
  ...
  onUpdate(dt, inp) {
    lakeMover.update(
      dt, inp, map, lakeScene.camera, lakeScene.tileW, lakeScene.tileH,
      engine.originX, engine.originY, canvas.width, canvas.height,
      lakeHero.position.x, lakeHero.position.y
    );
    ...
    if (Math.hypot(lakeMover.velX, lakeMover.velY) > 0.002) {
      _rippleTimer += dt;
      if (_rippleTimer > 0.32) {
        _rippleTimer = 0;
        waveLake.addRipple(lakeHero.position.x, lakeHero.position.y);
        _spawnRipple(lakeScene, lakeHero.position.x, lakeHero.position.y);
      }
    } else { _rippleTimer = 0; }
  ...
  onDrawOverlay: (ctx, _w, _h, ts) => lakeMover.drawMarker(
    ctx, lakeScene.camera, lakeScene.tileW, lakeScene.tileH,
    engine.originX, engine.originY, ts
  ),
}));
...
mgr.register('deep', () => ({
  ...
  onUpdate(dt, inp) {
    deepMover.update(
      dt, inp, map, deepScene.camera, deepScene.tileW, deepScene.tileH,
      engine.originX, engine.originY, canvas.width, canvas.height,
      deepHero.position.x, deepHero.position.y
    );
  ...
  onDrawOverlay: (ctx, _w, _h, ts) => deepMover.drawMarker(
    ctx, deepScene.camera, deepScene.tileW, deepScene.tileH,
    engine.originX, engine.originY, ts
  ),
}));
...
function _spawnDust(scene: Scene, cols: number, rows: number): void {
  ...
  ps.addEmitter({
    maxParticles: 5, rate: 0, shape: 'point',
...
function _spawnDream(scene: Scene, cols: number, rows: number): void {
  ...
  ps.addEmitter({
    maxParticles: 3, rate: 0, shape: 'point',
...
function _spawnRipple(scene: Scene, x: number, y: number): void {
  ...
  ps.addEmitter({
    maxParticles: 12, rate: 0, shape: 'ring',
