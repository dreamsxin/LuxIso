...
| P2 | 1.5% 的像素门禁松到能漏掉一整个道具 | 实测而非估计：把 `mossy-boulder` 的半径从 19 改成 120 重新构建，690,816 个画布像素里只有 4,574 个变了——0.66%，低于 1.5% 阈值，门禁照常绿。要抓道具级回归需要按 fixture 分别设阈值，或把预算限定到区域;当前这个数只抓得住全场景级的变化 |
| P2 | 三张已提交基线现在约 0.14% 过期 | 下面那条 `maxZ` 修正让石头的阴影短了 935（`day-ne`）、976（`low-angle`）、919（`night-lanterns`）个像素。三者都过门禁,CI 不会红;下次跑 `webgl-baselines` 时吸收掉即可 |
