...
- Playwright 使用固定 Chromium/SwiftShader、1280×720、DPR 1 验证非空像素和跨帧稳定性；`day-ne` / `low-angle` / `night-lanterns` 三个 fixture 的基线已提交，golden diff 已作为阻断门禁生效（预算 2,500 像素，此前是 1.5% 比例 = 10,362 像素），其余六个仍只有启发式断言。
...
- 回到覆盖率地图上最薄的一块 `src/elements/**`。按 lcov 逐文件排下来，最低的三个是
  `Boulder`（12%）、`Chest`（17.5%）、`Cloud`（18.3%）——都不是逻辑没写，而是**整个
  draw 路径没有任何测试**。先补的是基础设施：`src/__tests__/helpers/canvas.ts` 提供一个
  会记录调用与属性写入的 2D 上下文（此前五个测试各自手搓过一份略有差异的 stub），
  于是测试可以断言"画了什么"，而不只是"没有抛异常"。
  第一个被它照出来的就是 **`_lastTs = 0` 哨兵的第十次出现**：`Cloud.update()` 用 0 当
  首帧哨兵，而 `Engine` 的首帧时间戳正好是 0，于是哨兵永不解除——**从 0 开始的场景里，
  云一辈子不飘**。同时它也缺时间回退的保护（`update(1000)` 后 `update(500)` 会把云往
...
- **`_lastTs = 0` 第十一、十二次出现，以及第一次不靠"下次注意"来防第十三次。**
  上一条记录写着"新写的时间累积代码必须用 `null` 做首帧哨兵"——这条规律已经成立十次，
  每一次都配了回归测试，而第十一次照样发布了。原因很直接：**模块级测试无法为一个还不
  存在的模块失败**。所以这次不再只修，而是把契约写成一份共享用例表
  `src/__tests__/FrameDeltaContract.test.ts`：每个自己从时间戳算 `dt` 的模块登记一个
  `{ tick(ts), read() }`，三条规则各断言一次。
  - 三条规则都是生产代码真实违反过的：① 时间戳 0 是普通的第一帧（该帧不推进，**下一帧
    拿到真实增量**）；② 时间倒流不得回退状态；③ 长间隔（隐藏标签页、断点）要钳制而不是
    积分。
  - 规则①的写法是关键：不能只断言"第一帧不动"，那样 `=== 0` 哨兵照样通过。要跑**两遍**
    ——一遍从 ts=0 开始，一遍从 ts=1000 开始，同样的 100ms 增量必须得到同样的推进量。
  - 第十一次：`ParticleSystem` 仍是 `_lastTs = 0` + `? 1/60 :`，且**完全没有** `Math.max(0, …)`。
    后者更糟：`Particle.update` 是 `life -= dt`，负 dt 会让粒子变年轻、倒着飘、永不消亡。
    实测对照：改回旧写法后契约表立刻在 ParticleSystem 上失败（1 个粒子 vs 5 个）。
  - 第十二次由这份新表**当场抓到**，而不是靠人再读一遍代码：`FloatingText` 有 `null` 哨兵
    （上一轮修过），却漏了 `Math.max(0, …)`——时间倒流会把飘字往下拽并反向淡入。它自己
    那 13 个用例全绿，共享表第一次运行就红。**这就是共享契约表相对模块级回归测试的增量。**
  - `src/main.ts` 的示例主循环也是同一模式（`_lastTs = 0` + `? 0.016 :`，无负值钳制），
    一并改掉。
...
- **覆盖率地图上最后一块 11% 的 `Boulder` 补完，顺手量出一处不能马上改的缺陷。**
  ...
  - **没有当场修**：`maxZ` 同时喂给 `depthSort` 与 `ShadowCaster`，而 `mossy-boulder`
    ...
    **把猜测变成钉住的事实，比留一句 TODO 有用。**
    （**后续已修好**，见本节最后一条"两条 `maxZ` 从'量出来'改成'修好了'"。）
...
- **最后一块大面积未覆盖的绘制体 `Chest` 补完（语句 14% → 100%）。**
  ...
  - **又量出一处同类缺陷**：`aabb.maxZ` 是常量 51.2px（`tileH=32` 下的箱体+盖厚，
    ...
    （**后续已修好**，且当时这条测量把"超出量"算大了——见本节最后一条。）
...
- **两条 `maxZ` 从"量出来"改成"修好了",顺带发现像素门禁其实抓不住道具级回归。**
  上一轮我把这两条推迟的理由写成"没有 GPU 无法本地验证",那句已更正;这一轮直接做完并本地实测。
  - `Boulder.aabb.maxZ` 从 `radius * 2` 改成 `radius * Boulder.SQUASH`。0.55 这个压扁系数
    现在是类上的常量,`draw` 的顶点计算和 `aabb` 共用同一个——**两个数再也不能各自漂移**,
    这比把 9.9 硬编码进去重要。顶点 0 恰好在 -90° 且半径系数为 1.0,所以"画出来的高度"
    是精确值而不是一个区间,用例因此从 band 改成 `toBeCloseTo`。
  - `Chest.aabb.maxZ` 改成跟着箱盖走:用 `draw` 那套同样的绕铰链旋转算出前沿抬升,
    `max(关闭时的盖厚, 抬升量)`。**关着时精确等于旧的 51.2**,所以关着的箱子像素一动不动。
  - **旧那条测量断言的比较方式是错的,我自己写的。** 它拿 `centre.y - maxZ` 去比画出来的
    最高 y——但箱子的菱形是子格 footprint,它的北角本身就在锚点上方 `HS * tileH = 12.16px`,
    **那是地平面偏移,不是高度**。两者相减,于是"超出量"被算成 30px,老实说只有约 13.6px。
    新用例显式减掉这个偏移,并在注释里写明"显然的那个比较是错的"。
  - **门禁实测三连**：(1) 打开 `CI=1` 后三张基线全过——但这不能说明什么;
    (2) 控制实验:把石头半径从 19 调到 34,门禁**照样绿**;(3) 交叉实验:让 `day-ne` 去比
    `night-lanterns.png`,报 0.16 差异并失败——所以**匹配器是活的,阈值是松的**。
    把 `maxDiffPixelRatio` 临时设成 0 量到了准确数字:半径 120 的石头只动 4,574/690,816
    = **0.66%**,而 1.5% 的门槛放它过去。**一整个道具可以变成 6 倍大而门禁不响。**
  - 这两条修正自己的实际影响也量了:935 / 976 / 919 像素（约 0.14%），全部过门禁。
    也就是说三张基线现在轻微过期但不会让 CI 变红,下次 `webgl-baselines` 吸收即可。
    这比"卡住一个正确性修复去等一个我跑不了的 workflow"更合适,而且是门禁自己说它在容差内。
  - 用例数不变（1074),`src/elements/**` 与 `src/math/**` 阈值不动——这轮改的是数值正确性,
    不是覆盖面。
- **像素门禁从"1.5% 比例"换成"2,500 像素绝对预算",并且量清了它到底能抓什么。**
  上一条把"门禁太松"记成了 P2,这一条把它做掉,顺便得到一个比原判断更准的结论。
  - **换成绝对数的依据是确定性**:同一份构建连跑两次,三个 fixture 报出的差异数完全一样
    （935 / 976 / 919）。渲染是逐位确定的,所以绝对像素数是个诚实的量;比例那版把
    10,362 像素的预算包装成"1.5%",听起来严格,实际是把一整个道具的变化放进了容差。
  - **控制实验**:把预算临时设成 900,九个 fixture 里**恰好那三个被门禁的失败**,
    另外六个照常通过——参数确实生效,门禁确实在比。再设回 2,500,十一个测试全绿。
  - **但更重要的结论是收紧治不了根本问题**。石头半径 19→34（几乎翻倍）只动 1,102 像素,
    而一次合法的阴影修正动 935。**在 690,816 像素的画布上,这两件事只差 167 个像素**——
    任何全局预算都无法既放过后者又抓住前者。道具级保护需要裁剪区域的基线,
    或者一个可计算的不变量（比如按 `propRadius` 反推该有多少像素属于石头）。
    我把这条写进 ACCEPTANCE.md 和两份 README 的 P2,而不是假装阈值调对了就安全了。
  - 2,500 是个 ratchet,和 `vitest.config.ts` 里的覆盖率地板同一个道理:设在当前已知
    良好差异之上,只准往下收。眼下三张基线因为 `maxZ` 修正而差约 1,000 像素,
    等 `webgl-baselines` 重生成后这个差归零,预算应当降到几百。
- **道具级保护改到抽取层,顺手撞出一个双后端不一致:同一块石头两个渲染器画得不一样。**
  上一条说"整画布预算结构上保护不了单个道具",这一条把那份工作放到它该在的地方。
  - **`RenderSnapshot` 是渲染器中立的数据**,所以道具轮廓可以精确量出来:不需要浏览器、
    不需要基线、不需要阈值。半径翻倍就是轮廓翻倍,`toBeCloseTo(2)` 即可——而这正是
    像素门禁怎么调都做不到的判别。新用例 `WebGLPropParity.test.ts` 就干这件事。
  - **量的第一件事就翻出问题**:GL 抽取里的石头是一个"中心在锚点上方 `0.42r`、竖半径
    `0.72r`"的椭圆,向上到 `1.14r`、向下到 `0.30r`;而 Canvas2D 画的是以锚点为中心、
    上下各 `0.55r` 的压扁多边形。**同一个 `Boulder`,两个后端的形状和高度都不一样,
    而 `aabb.maxZ` 谁也不匹配**（修正前是 `2r`,把两个都高估了）。
    `webgl-next` 的 ROADMAP 第 2 阶段写的就是"几何一致性",这条正落在那里。
  - 已把抽取改成同一套:椭圆以锚点为中心、竖半径 `radius * Boulder.SQUASH`,高光面
    收在椭圆内。于是 **2D 轮廓、GL 轮廓、`aabb.maxZ` 三者同源**,用例把三者两两钉住。
  - **像素代价实测**:三张 fixture 的差异从 935 / 976 / 919 变成 1,052 / 1,028 / 919,
    仍远在 2,500 预算内,所以这个修正可以直接发。
