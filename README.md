...
| P2 | The 1.5% pixel gate is loose enough to miss a whole prop | Measured, not guessed: rebuilding `mossy-boulder` at 120 px radius instead of 19 changes 4,574 of 690,816 canvas pixels — 0.66%, under the 1.5% ratio, so the gate stays green. Per-fixture thresholds, or a diff budget scoped to a region, would catch prop-level regressions; the current number only catches something scene-wide |
| P2 | The three committed baselines are ~0.14% stale | The `maxZ` corrections below shortened the boulder's shadow by 935 (`day-ne`), 976 (`low-angle`) and 919 (`night-lanterns`) pixels. All pass the gate, so CI stays green; the next `webgl-baselines` run should absorb the difference |
