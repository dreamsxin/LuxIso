...
| P2 | `Chest` is the last large uncovered draw body (14% statements) | `src/__tests__/helpers/canvas.ts` gives the draw paths a recording 2D context, which took `Cloud` and then `Boulder` (11% → 100%) to covered. `Chest`'s 250-line `draw` is what remains |
